...
namespace OperatorsOverloading
{
    ...
    public abstract class List<TValue>
    {
        ...
        {
            var elem = new TValue[list.Length];
            int n = 0;
            foreach(var i in list.Flatten())//maybe
            {
                elem[n] = i.Head;
                n++;
            }
        ...
        public static List<TValue> operator -(List<TValue> list1, List<TValue> list2)
        {
            return List.From(((TValue[]) list1).Where(a => !((TValue[])list2).Contains(a)));
        }
